...
struct
  ...
  let test_is_empty () = 
      ...
      assert (not (is_empty (singleton key_generator)));
      assert (not (insert key_generator (insert key_generator empty)));
      ()


  let test_singleton () =
    let x = key_generator in
  ...
  (* searches a list and uses D.compare to find a specific element *)
